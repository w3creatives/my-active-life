...
class TeamsController extends BaseController
{
    ...
    public function achievements(Request $request): JsonResponse
    {
       ...
       $year = $request->year?$request->year:Carbon::now()->format('Y');
        ...
        // Clear cache if cache_clear parameter is set to 1
        if($request->cache_clear == 1){
     ...
     //  dd($team,$user->id);
       ...
       if(is_null($team)){
        ...
        $users = $users->map(function($item) use($request, $startOfYear, $endOfYear, $year, $team){
            ...
            // Calculate today's points for individual user
            $today = Carbon::now()->setTimezone($user->time_zone_name??'UTC')->format('Y-m-d');
            $todayUserPoints = $item->user->points()
                ->where('event_id', $request->event_id)
                ->whereDate('date', $today)
                ->sum('amount') ?? 0;
            
            // Calculate this week's points for individual user
            $startOfWeek = Carbon::now()->startOfWeek()->format('Y-m-d');
            $endOfWeek = Carbon::now()->endOfWeek()->format('Y-m-d');
            $thisWeekUserPoints = $item->user->points()
                ->where('event_id', $request->event_id)
                ->whereBetween('date', [$startOfWeek, $endOfWeek])
                ->sum('amount') ?? 0;
                
            // Calculate this month's points for individual user
            $startOfMonth = Carbon::now()->startOfMonth()->format('Y-m-d');
            $endOfMonth = Carbon::now()->endOfMonth()->format('Y-m-d');
            $thisMonthUserPoints = $item->user->monthlyPoints()
                ->selectRaw('SUM(amount) as totalPoint')
                ->hasEvent($request->event_id)
           ...
           $user['todayPoints'] = ['point' => $todayUserPoints, 'date' => $today];
           $user['thisWeekPoints'] = ['point' => $thisWeekUserPoints, 'start_date' => $startOfWeek, 'end_date' => $endOfWeek];
           $user['thisMonthPoints'] = ['point' => $thisMonthUserPoints, 'start_date' => $startOfMonth, 'end_date' => $endOfMonth];
        ...
        if ( !empty($teamSettings['chutzpah_factors']) && is_array($teamSettings['chutzpah_factors']) ) {
            // get first element of chutzpah_factors
            $first = array_shift($teamSettings['chutzpah_factors']);
        
            if (is_array($first) && array_key_exists($eventName, $first)) {
                $chutzpahFactor = $first[$eventName];
            }
        }
        
        // Calculate team total completed and remaining miles
        $teamTotalDistance = $team->totalPoints()->where('event_id', $request->event_id)->sum('amount') ?? 0;
        
        $teamTotalGoal = 0;
        if($chutzpahFactor) {
            $teamTotalGoal = $team->event->total_points * $chutzpahFactor;
        } else {
            $teamTotalGoal = $team->event->total_points ?? 0;
        }

        // Ensure completed distance doesn't exceed the goal
        if($teamTotalDistance > $teamTotalGoal && $teamTotalGoal > 0){
            $teamTotalDistance = $teamTotalGoal;
        }
        
        $teamRemainingDistance = max(0, $teamTotalGoal - $teamTotalDistance);
        $teamProgressPercentage = $teamTotalGoal > 0 ? ($teamTotalDistance / $teamTotalGoal) * 100 : 0;
        
        $teamMilesData = [
            'total_miles' => round($teamTotalGoal, 2),
            // 'total_miles' => round($teamTotalDistance, 2),
            'completed_miles' => round($teamTotalDistance, 2),
            'remaining_miles' => round($teamRemainingDistance, 2),
            'progress_percentage' => round($teamProgressPercentage, 2)
        ];
        
        // Calculate today's points for team
        $today = Carbon::now()->setTimezone($user->time_zone_name??'UTC')->format('Y-m-d');
        $teamMemberIds = $team->memberships()->where('event_id', $request->event_id)->pluck('user_id');
        $todayTeamPoints = UserPoint::whereIn('user_id', $teamMemberIds)
            ->where('event_id', $request->event_id)
            ->whereDate('date', $today)
            ->sum('amount') ?? 0;
            
        // Calculate this week's points for team
        $startOfWeek = Carbon::now()->startOfWeek()->format('Y-m-d');
        $endOfWeek = Carbon::now()->endOfWeek()->format('Y-m-d');
        $thisWeekTeamPoints = $team->weeklyPoints()->selectRaw('SUM(amount) as totalPoint')
            ->hasEvent($request->event_id)
            ->whereBetween('date', [$startOfWeek, $endOfWeek])
            ->first()->totalpoint ?? 0;
            
        // Calculate this month's points for team
        $startOfMonth = Carbon::now()->startOfMonth()->format('Y-m-d');
        $endOfMonth = Carbon::now()->endOfMonth()->format('Y-m-d');
        $thisMonthTeamPoints = $team->monthlyPoints()->selectRaw('SUM(amount) as totalPoint')
            ->hasEvent($request->event_id)
            ->whereBetween('date', [$startOfMonth, $endOfMonth])
            ->first()->totalpoint ?? 0;

        // Prepare time-based points data
        $todayPoints = ['point' => $todayTeamPoints, 'date' => $today];
        $thisWeekPoints = ['point' => $thisWeekTeamPoints, 'start_date' => $startOfWeek, 'end_date' => $endOfWeek];
        $thisMonthPoints = ['point' => $thisMonthTeamPoints, 'start_date' => $startOfMonth, 'end_date' => $endOfMonth];
        
         Cache::put($cacheName, compact('achievements','teamMilesData','yearlyPoints','todayPoints','thisWeekPoints','thisMonthPoints','users'), now()->addHours(2));
       
       return $this->sendResponse(compact('achievements','teamMilesData','yearlyPoints','todayPoints','thisWeekPoints','thisMonthPoints','users'),'Response');
    ...
    public function teamToFollowList(Request $request): JsonResponse 
    {
       ...
       ->through(function ($team) use($user,$eventId){
            ...
            $followRequest = $team->followerRequests()->where(["prospective_follower_id" => $user->id,"event_id" => $eventId])->first();
            
            $isFollowing = $team->followers()->where(['follower_id' => $user->id, 'event_id' => $eventId])->count();
        
            if($isFollowing) {
                $followStatus = 'request_to_follow_approved';
