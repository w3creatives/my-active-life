...
class ProfilesController extends BaseController
{
    ...
    public function store(Request $request) : JsonResponse
    {
        ...
        $user->fill($request->only(['email','first_name','last_name',
            ...
            'shirt_size','preferred_event_id']))->save();
    ...
    public function findSetting(Request $request) : JsonResponse 
    {
        ...
        switch($request->setting) {
            ...
            case 'rty_mileage_goal':
                ...
                $totalDays = Carbon::parse($participation->subscription_end_date)->diffInDays(Carbon::parse($event->start_date));
                ...
                $userTotalPointDays = $user->points()->where('event_id',$event->id)->where('date','>=',Carbon::parse($event->start_date)->format('Y-m-d'))->where('date','<=', Carbon::now()->format('Y-m-d'))->groupBy('date')->count();
                 
                $userTotalPoints = $userPoint->total_mile;
                
                $totalDayTillToday = Carbon::now()->diffInDays(Carbon::parse($event->start_date));
                
                $totalRemainingDays = $totalDays - $userTotalPointDays;
