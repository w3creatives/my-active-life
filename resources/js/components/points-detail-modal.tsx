...
export function PointsDetailModal({ isOpen, onClose, date, eventId, activeModality }: PointsDetailModalProps) {
    ...
    useEffect(() => {
        if (isOpen && date && eventId) {
            ...
            axios.get(route('user.daily.points', {
                date: formattedDate,
                event_id: eventId,
    ...
    const getPointAmount = (dataSourceId: string, modality: string): number => {
        if (!pointsData || !pointsData.points[dataSourceId]) return 0;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent
                className="sm:max-w-3xl max-h-[95vh] flex flex-col"
                aria-describedby="points-detail-description"
            >
                <DialogHeader>
                    <DialogTitle className="text-xl">Points for {date ? format(date, 'MMMM d, yyyy') : ''}</DialogTitle>
                    <DialogDescription id="points-detail-description">
                                            ...
                                            <div key={`garmin-${modality}`} className="space-y-1">
                                                ...
                                                <div className="flex items-center">
                                                    <Input
                                                        type="number"
                                                        step="0.01"
                                                        defaultValue={formatValue(getPointAmount('3', modality))}
                                                        className="w-full"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div key={`fitbit-${modality}`} className="space-y-1">
                                                ...
                                                <div className="flex items-center">
                                                    <Input
                                                        type="number"
                                                        step="0.01"
                                                        defaultValue={formatValue(getPointAmount('2', modality))}
                                                        className="w-full"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div key={`strava-${modality}`} className="space-y-1">
                                                ...
                                                <div className="flex items-center">
                                                    <Input
                                                        type="number"
                                                        step="0.01"
                                                        defaultValue={formatValue(getPointAmount('4', modality))}
                                                        className="w-full"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div key={`apple-${modality}`} className="space-y-1">
                                                ...
                                                <div className="flex items-center">
                                                    <Input
                                                        type="number"
                                                        step="0.01"
                                                        defaultValue={formatValue(getPointAmount('5', modality))}
                                                        className="w-full"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div key={`manual-${modality}`} className="space-y-1">
                                                ...
                                                <div className="flex items-center">
                                                    <Input
                                                        type="number"
                                                        step="0.01"
                                                        defaultValue={formatValue(getPointAmount('1', modality))}
                                                        className="w-full"
                                                    />
                                                </div>
                                            </div>
