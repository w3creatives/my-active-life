                ...
                <DialogDescription className="space-y-6 py-4 text-base">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <Button
                      ...
                      className={`flex h-auto flex-col items-center justify-center gap-1 px-4 py-2 text-wrap ${deleteData === 'no' ? 'ring-primary ring-2' : ''}`}
                      onClick={() => setDeleteData('no')}
                    >
                      <div className="text-lg font-semibold text-wrap">Preserve Miles</div>
                      <div className="text-center text-sm text-wrap">Keep all your previously synced miles</div>
                    </Button>
                    <Button
                      type="button"
                      variant={deleteData === 'yes' ? 'default' : 'outline'}
                      className={`flex h-auto flex-col items-center justify-center gap-1 px-4 py-2 text-wrap ${deleteData === 'yes' ? 'ring-primary ring-2' : ''}`}
                      onClick={() => setDeleteData('yes')}
                    >
                      <div className="text-lg font-semibold text-wrap">Delete Miles</div>
                      <div className="text-center text-sm text-wrap">Remove all miles synced from this device</div>
                    </Button>
                  </div>
                </DialogDescription>
                <DialogFooter className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <Button variant="outline" onClick={() => setIsDisconnectDialogOpen(false)} className="w-full sm:w-auto">
                    Cancel
                  </Button>
                  <Button variant="destructive" onClick={confirmDisconnect} disabled={isDisconnecting} className="w-full cursor-pointer sm:w-auto">
                    {isDisconnecting ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        Disconnecting...
