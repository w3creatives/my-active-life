      ...
      <CardContent>
        <div className="space-y-6">
                ...
                <div key={index} className="space-y-1">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className={`pr-10 pl-10 ${fieldError ? 'border-red-500 focus-visible:border-red-500' : ''}`}
                    />
                    {emails.length > 1 && (
                      <button
                        type="button"
          ...
          <div className="flex justify-center gap-2">
            <Button type="button" variant="outline" onClick={addEmailField} disabled={emails.length >= 20} className="flex items-center gap-2">
              <Plus className="h-4 w-4" />
              Add Another Email ({emails.length}/20)
            </Button>
            {emails.some((email) => email.trim() !== '') && (
              <Button type="button" variant="outline" onClick={clearAllEmails} className="flex items-center gap-2 text-gray-600">
                Clear All
              </Button>
            )}
          </div>

          <Button variant="default" onClick={handleInvite} className="w-full">
            Invite New Members
          </Button>

          <div className="rounded-md bg-gray-50 p-4 text-sm text-gray-500">
            <p>
              Just a heads up, only a team captain can approve requests to join the team. Any team member can invite new team members. When
              participants accept the invitation they will show up automagically in your team.
            </p>
          </div>
        </div>
      </CardContent>
